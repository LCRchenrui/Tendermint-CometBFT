...
func (c *Client) do(ctx context.Context, method, path string, form url.Values) error {
	...
	if err := validateWriteResponse(raw); err != nil {
		return fmt.Errorf("nacos %s %s -> invalid response: %w; body=%s", method, path, err, string(raw))
	}
	return nil
}
...
func decodeInstanceList(body []byte) ([]Instance, error) {
	...
	var outV2 v2Resp
	if err := json.Unmarshal(body, &outV2); err == nil {
...
func validateWriteResponse(body []byte) error {
	...
	if err := json.Unmarshal(body, &boolResp); err == nil {
		...
		return fmt.Errorf("response is false")
	}
	...
	if err := json.Unmarshal(body, &resp); err == nil {
		...
		if resp.Code == 0 && strings.EqualFold(strings.TrimSpace(resp.Message), "success") {
			return nil
		}
		if resp.Message != "" {
